...
export function Navbar({ activeTab, onTabChange }: NavbarProps) {
  ...
  return (
    <motion.header
      animate={{ height: collapsed ? 8 : 52 }}
      transition={{ duration: 0.4, ease }}
      className="fixed top-0 left-0 right-0 z-50 overflow-hidden border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-glass)] backdrop-blur-[20px] backdrop-saturate-[1.8]"
    >
      <AnimatePresence>
        {!collapsed && (
          <motion.nav
      ...
      <button
        ...
        className="absolute top-1 right-4 z-10 flex h-6 w-6 items-center justify-center text-[var(--color-pumice)] transition-colors duration-200 hover:text-[var(--color-steam)]"
